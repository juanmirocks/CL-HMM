...
(defun protocol-experiment (results-folder &key
                                             (training-data-path (pwd "test/resources/sample_feldman.txt"))
                                             (training-size nil)
                                             (test-data-path (pwd "test/resources/test_feldman.txt"))
                                             (num-translations 1000)
                                             (num-states-list '(1 2 4 8 16 32))
                                             (em-num-iterations 5)
                                             (em-max-times 100)
                                             (verbose-bws t))

  (multiple-value-bind (in L R) (read-pair-observations-file training-data-path)
    (loop for num-states in num-states-list do
         ...
         (let* ((phmm (make-random-phmm num-states (length L) (length R) :L-list L :R-list R))
                ...
                (training-size (or training-size (length training-data)))
                (training-data (subseq training-data 0 training-size))
                (test-data (mapcar #'(lambda (o) (cbook-encode phmm o)) (read-pair-observations-file test-data-path))))
           (multiple-value-bind (best loglikelihood)
               (hmm-estimate-bws phmm training-data :threshold 0.05 :verbose-bws verbose-bws :starting-noise 0 :max-times em-max-times :iterations em-num-iterations :confidence 0)
